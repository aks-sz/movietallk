...
Tools:
└── s5p6818-sdmmc.sh -- For burnning ubootpak.bin to external booting SDCARD
...
Android系统：
1、准备一张SD卡，在其根目录下建立x6818-android文件夹
2、拷贝镜像文件：ubootpak.bin boot.img system.img至x6818-android目录下,如果拷贝userdata.img，cache.img，recovery.img则一便更新。
3、并在x6818-android目录下，创建环境变量默认配置文件env.txt,不存在，则不更新系统环境变量
...
QT系统：
1、准备一张SD卡，在其根目录下建立x6818-qt文件夹
2、拷贝镜像文件：ubootpak.bin boot.img qt-rootfs.img至x6818-qt目录下
3、并在x6818-qt目录下，创建环境变量默认配置文件env.txt,不存在，则不更新系统环境变量
4、env.txt文件内容配置示例:(需正确设置环境变量,在文件末尾需要保留一空行)
...
UBUNTU系统:
1、准备一张SD卡，在其根目录下建立x6818-ubuntu文件夹
2、拷贝镜像文件：ubootpak.bin boot.img ubuntu-rootfs.tar.bz2至x6818-ubuntu目录下
3、并在x6818-ubuntu目录下，创建环境变量默认配置文件env.txt,不存在，则不更新系统环境变量
4、env.txt文件内容配置示例:(需正确设置环境变量,在文件末尾需要保留一空行)
	bootcmd=ext4load mmc 2:1 0x48000000 uImage;bootm 0x48000000
...
env set bootargs "root=/dev/mmcblk0p2 rootfstype=ext4 lcd=vs070cxn tp=gslx680-linux v4l=remove"
env save
